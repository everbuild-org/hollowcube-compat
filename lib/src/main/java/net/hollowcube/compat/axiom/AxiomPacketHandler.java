...
final class AxiomPacketHandler {
    private static final Logger log = LoggerFactory.getLogger(AxiomPacketHandler.class);
    ...
    static void onSetWorldProperty(@NotNull Player player, @NotNull AxiomServerboundSetWorldPropertyPacket packet) {
        ...
        try {
            ...
            log.error("onSetWorldProperty", e);
        }
    }
    ...
    static void onSpawnEntities(@NotNull Player player, @NotNull AxiomServerboundSpawnEntitiesPacket packet) {
        ...
        for (var entry : packet.entries()) {
            ...
            try {
                ...
                log.error("Error while trying to spawn entity {}", entry.id(), e);
    ...
    private static @org.jspecify.annotations.Nullable Pos getPos(AxiomServerboundModifyEntitiesPacket.Entry entry, Entity entity) {
        var pos = entry.pos();
        if (pos != null) {
