...
import net.hollowcube.terraform.TaskApplicator;
...
public class AxiomModule {
    private final TaskApplicator applicator;
    ...
    public AxiomModule(TaskApplicator applicator) {
    ...
    private void handleBufferApplication(@NotNull AxiomApplyBufferEvent event) {
        ...
        var task = new Task.Builder("axiom-" + event.id(), event.getInstance())
                .metadata()
                .buffer(AxiomTerraformBuffer.of(buffer))
                .ephemeral()
                .get();
        applicator.submitTask(task);
