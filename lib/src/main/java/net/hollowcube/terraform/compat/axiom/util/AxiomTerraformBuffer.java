...
public record AxiomTerraformBuffer(
    ...
    private static final Logger log = LoggerFactory.getLogger(AxiomTerraformBuffer.class);
    ...
    public static AxiomTerraformBuffer of(AxiomBlockBuffer buffer) {
        ...
        if (!exceptions.isEmpty()) {
            ...
            log.error("Failed to decompress block entities", exception);
        }
    ...
    private record AxiomPalette(
        ...
        public @Nullable Block get(int x, int y, int z) {
            ...
            var blockEntity = OpUtils.map(block.blockEntityType(), v -> v.key().asString());
